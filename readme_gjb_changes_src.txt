Had to change the following files inside tensorflow library (see ./tf_changes):
 ...
 multi_gpu_utils.py to support xla_gpu
...
To find location of TensorFlow files:
  ...
  4. backend.py should be located in $TENSOR_DIR/python/keras
     ...
     4.2 copy new (cp ./tf_changes/backend.py $TENSOR_DIR/python/keras/backend.py)
  5. mult_gpu_utils.py should be located in $TENSOR_DIR/python/keras/utils
     5.1 move original
     5.2 copy new
